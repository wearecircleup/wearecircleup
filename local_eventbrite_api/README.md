# Local Eventbrite CRUD API

Local FastAPI interface for creating, listing, reading, updating and deleting Eventbrite events. Its interactive test interface is at `/docs`.

This whole directory is intentionally ignored by Git. Keep it local and do not expose the server to the internet because it uses an Eventbrite private token.
